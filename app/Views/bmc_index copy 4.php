...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* History tab bar hide scrollbar */
...
<body style="height:100vh;overflow:hidden;display:flex;flex-direction:column;">
    ...
    <!-- Top navbar -->
    <header
        style="height:52px;background:var(--surface);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;padding:0 20px;flex-shrink:0;z-index:10;">
        <div style="display:flex;align-items:center;gap:10px;">
            <div style="background:var(--accent);padding:7px;border-radius:9px;display:flex;">
                <i data-lucide="terminal" style="width:16px;height:16px;color:#fff;"></i>
            </div>
            <span class="mono" style="font-size:16px;font-weight:700;">Parse<span
        ...
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div
                style="width:30px;height:30px;border-radius:50%;background:var(--surface2);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;">
                <i data-lucide="bell" style="width:13px;height:13px;color:var(--muted);"></i>
            </div>
            <div
                style="width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--accent2));display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#fff;">
                JD</div>
        </div>
    </header>
    ...
    <div style="flex:1;display:flex;overflow:hidden;">
        ...
        <!-- Sidebar history -->
        <aside id="sidebar"
            style="width:240px;flex-shrink:0;background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;overflow:hidden;">
            <!-- Sidebar header -->
            <div
                style="padding:14px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;">
                <span class="mono" style="font-size:10px;color:var(--muted);letter-spacing:.1em;">HISTORY</span>
                <button onclick="switchView('analyzer')" id="btn-new-analysis"
                    style="display:flex;align-items:center;gap:4px;background:var(--accent);border:none;border-radius:6px;padding:5px 10px;cursor:pointer;color:#fff;">
                    <i data-lucide="plus" style="width:11px;height:11px;"></i>
                    <span class="mono" style="font-size:9px;">NEW</span>
                </button>
            </div>

            <!-- History list -->
            <div id="history-list" style="flex:1;overflow-y:auto;padding:8px;">
                <!-- injected by loadHistory() -->
                <div id="history-empty" style="padding:24px 12px;text-align:center;opacity:0.4;">
                    <i data-lucide="inbox" style="width:24px;height:24px;color:var(--muted);margin-bottom:8px;"></i>
                    <p class="mono" style="font-size:10px;color:var(--muted);margin:0;">Belum ada history</p>
                </div>
            </div>
            ...
            <div style="padding:10px 16px;border-top:1px solid var(--border);">
                <button onclick="loadHistory()"
                    style="width:100%;background:transparent;border:1px solid var(--border);border-radius:8px;padding:7px;cursor:pointer;color:var(--muted);display:flex;align-items:center;justify-content:center;gap:6px;">
                    <i data-lucide="refresh-cw" style="width:12px;height:12px;"></i>
                    <span class="mono" style="font-size:9px;">REFRESH</span>
                </button>
            </div>
        </aside>

        <!-- Main content -->
        <main style="flex:1;overflow-y:auto;padding:24px 28px 40px;">

            <!-- View: Analyzer (upload + analisis baru) -->
                ...
                <div style="display:flex;flex-direction:column;gap:20px;">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;align-items:stretch;">
                        ...
                        <div class="panel" style="padding:20px;">
                            ...
                            <p style="font-size:10px;color:var(--muted);margin-top:12px;line-height:1.6;">Mode
                                mempengaruhi fokus analisis BMC yang dihasilkan AI.</p>
                        </div>
                        ...
                        <div class="panel" style="padding:20px;display:flex;flex-direction:column;gap:16px;">
                            ...
                            <button id="extract-btn" class="btn-primary" style="width:100%;justify-content:center;"
                                disabled>
                    ...
                    <div class="panel" id="results-area"
                        ...
                        <div id="empty-state"
                            ...
                            <p class="mono" style="font-size:12px;color:var(--muted);">Waiting for execution command...
                            </p>
                        </div>
                        ...
                        <div id="loading-state"
                            ...
                            <div style="display:flex;flex-direction:column;gap:8px;">
                                <div style="display:flex;justify-content:space-between;align-items:center;">
                                    <span class="mono" style="font-size:10px;color:var(--muted);"
                                        id="progress-label">STEP 1 / 3</span>
                                    <span class="mono" style="font-size:10px;color:var(--accent);"
                            ...
                            <div style="display:flex;flex-direction:column;gap:10px;" id="steps-list">
                                <div class="step-item" data-step="0"
                                    ...
                                    <span class="step-icon mono"
                                        style="font-size:11px;width:18px;text-align:center;">○</span>
                        ...
                        <div id="tabs-area" style="display:none;flex-direction:column;flex:1;overflow:hidden;">
                            ...
                            <!-- Tab bar -->
                            <div style="padding:12px 0 0;border-bottom:1px solid var(--border);position:relative;">

                                <!-- Left fade + arrow -->
                                <div id="tab-scroll-left-wrap"
                                    style="display:none;position:absolute;left:0;top:0;bottom:13px;width:48px;z-index:3;pointer-events:none;align-items:center;justify-content:flex-start;">
                                    <div
                                        style="position:absolute;inset:0;background:linear-gradient(to right,var(--surface) 40%,transparent);border-radius:0;">
                                    </div>
                                    <button id="tab-scroll-left"
                                        onclick="document.getElementById('tab-bar').scrollBy({left:-200,behavior:'smooth'})"
                                        style="position:relative;z-index:1;pointer-events:all;width:26px;height:26px;border-radius:7px;border:1px solid var(--border);background:var(--surface);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text);font-size:16px;line-height:1;padding:0;margin-left:8px;box-shadow:0 1px 4px rgba(0,0,0,0.15);">‹</button>
                                </div>

                                <!-- Right fade + arrow -->
                                <div id="tab-scroll-right-wrap"
                                    style="display:none;position:absolute;right:0;top:0;bottom:13px;width:48px;z-index:3;pointer-events:none;align-items:center;justify-content:flex-end;">
                                    <div
                                        style="position:absolute;inset:0;background:linear-gradient(to left,var(--surface) 40%,transparent);border-radius:0;">
                                    </div>
                                    <button id="tab-scroll-right"
                                        onclick="document.getElementById('tab-bar').scrollBy({left:200,behavior:'smooth'})"
                                        style="position:relative;z-index:1;pointer-events:all;width:26px;height:26px;border-radius:7px;border:1px solid var(--border);background:var(--surface);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text);font-size:16px;line-height:1;padding:0;margin-right:8px;box-shadow:0 1px 4px rgba(0,0,0,0.15);">›</button>
                                </div>

                                <div id="tab-bar"
                                    style="display:flex;gap:6px;padding:0 20px 8px;overflow-x:auto;scroll-behavior:smooth;-webkit-overflow-scrolling:touch;">
                                    <!-- tabs injected here -->
                                </div>

                                <!-- Scrollbar track -->
                                <div id="tab-scrollbar-track"
                                    style="height:2px;background:var(--surface2);border-radius:99px;margin:0 20px 6px;overflow:hidden;display:none;">
                                    <div id="tab-scrollbar-thumb"
                                        style="height:100%;background:var(--accent);border-radius:99px;width:30%;position:relative;left:0%;transition:left 0.1s,width 0.1s;">
                                    </div>
                                </div>
                            </div>
            ...
            <!-- View: History detail (hasil dari DB, tanpa upload UI) -->
            <div id="view-history" style="display:none;flex-direction:column;height:100%;">

                <!-- History detail header -->
                <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
                    <button onclick="switchView('analyzer')"
                        style="display:flex;align-items:center;gap:6px;background:var(--surface2);border:1px solid var(--border);border-radius:8px;padding:7px 12px;cursor:pointer;color:var(--muted);">
                        <i data-lucide="arrow-left" style="width:13px;height:13px;"></i>
                        <span class="mono" style="font-size:10px;">BACK</span>
                    </button>
                    <div>
                        <p id="history-detail-title" class="mono"
                            style="font-size:11px;font-weight:700;margin:0;color:var(--text);"></p>
                        <p id="history-detail-meta" class="mono" style="font-size:9px;color:var(--muted);margin:0;"></p>
                    </div>
                </div>

                <!-- Results area — sama persis dengan analyzer, tapi tanpa upload -->
                <div class="panel" style="flex:1;display:flex;flex-direction:column;overflow:hidden;min-height:500px;">

                    <!-- Loading state (reused) -->
                    <div id="history-loading"
                        style="display:none;flex:1;align-items:center;justify-content:center;gap:10px;">
                        <span class="cursor"></span>
                        <span class="mono" style="font-size:11px;color:var(--muted);">Loading...</span>
                    </div>

                    <!-- Tabs + Content (reused structure) -->
                    <div id="history-tabs-area" style="display:flex;flex-direction:column;flex:1;overflow:hidden;">
                        <div style="padding:12px 0 0;border-bottom:1px solid var(--border);position:relative;">
                            <div id="history-tab-scroll-left-wrap"
                                style="display:none;position:absolute;left:0;top:0;bottom:13px;width:48px;z-index:3;pointer-events:none;align-items:center;justify-content:flex-start;">
                                <div
                                    style="position:absolute;inset:0;background:linear-gradient(to right,var(--surface) 40%,transparent);">
                                </div>
                                <button
                                    onclick="document.getElementById('history-tab-bar').scrollBy({left:-200,behavior:'smooth'})"
                                    style="position:relative;z-index:1;pointer-events:all;width:26px;height:26px;border-radius:7px;border:1px solid var(--border);background:var(--surface);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text);font-size:16px;margin-left:8px;box-shadow:0 1px 4px rgba(0,0,0,0.15);">‹</button>
                            </div>
                            <div id="history-tab-scroll-right-wrap"
                                style="display:none;position:absolute;right:0;top:0;bottom:13px;width:48px;z-index:3;pointer-events:none;align-items:center;justify-content:flex-end;">
                                <div
                                    style="position:absolute;inset:0;background:linear-gradient(to left,var(--surface) 40%,transparent);">
                                </div>
                                <button
                                    onclick="document.getElementById('history-tab-bar').scrollBy({left:200,behavior:'smooth'})"
                                    style="position:relative;z-index:1;pointer-events:all;width:26px;height:26px;border-radius:7px;border:1px solid var(--border);background:var(--surface);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text);font-size:16px;margin-right:8px;box-shadow:0 1px 4px rgba(0,0,0,0.15);">›</button>
                            </div>
                            <div id="history-tab-bar"
                                style="display:flex;gap:6px;padding:0 20px 8px;overflow-x:auto;scroll-behavior:smooth;scrollbar-width:none;">
                            </div>
                            <div id="history-tab-scrollbar-track"
                                style="height:2px;background:var(--surface2);border-radius:99px;margin:0 20px 6px;overflow:hidden;display:none;">
                                <div id="history-tab-scrollbar-thumb"
                                    style="height:100%;background:var(--accent);border-radius:99px;width:30%;position:relative;left:0%;transition:left 0.1s,width 0.1s;">
                                </div>
                            </div>
                        </div>
                        <div id="history-tab-content" style="flex:1;overflow-y:auto;padding:24px;">
                            <div
                                style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:200px;opacity:0.3;">
                                <i data-lucide="mouse-pointer-click"
                                    style="width:32px;height:32px;color:var(--muted);margin-bottom:8px;"></i>
                                <p class="mono" style="font-size:11px;color:var(--muted);">Pilih history dari sidebar
                                </p>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    document.querySelectorAll('.mode-pill').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.getElementById('active-mode-label').textContent = labels[selectedMode] || selectedMode
                .toUpperCase();
        });
    });
    ...
    function renderTabContent(key, content = null) {
        content = content || document.getElementById('tab-content');
        content.innerHTML = '';
        ...
        const blok = analysisData.find(d => d.type === 'blok' && normalizeBlocKey(d.blok) === key);
        if (!blok) return;
    ...
    // Urutan blok BMC yang benar sesuai prompt backend
    const BMC_ORDER = ['__kesimpulan__', 'Value Proposition', 'Customer Segments', 'Channels', 'Customer Relationships',
        'Revenue Streams', 'Key Resources', 'Key Activities', 'Key Partnerships', 'Cost Structure'
    ];

    // Normalize blok name to match BMC_ORDER regardless of AI casing
    function normalizeBlocKey(rawKey) {
        if (!rawKey) return rawKey;
        const lower = rawKey.toLowerCase().trim();
        return BMC_ORDER.find(k => k.toLowerCase() === lower) || rawKey;
    }
    ...
    async function streamBMC(rawText) {
        ...
        let blockCount = 0;
        let estimatedTotal = 9;
        const API_ENDPOINT = "<?= base_url() ?>bmc/stream";

        const loadingEl = document.getElementById('loading-state');
        typingArea.textContent = '';
        typingArea.closest('div').style.display = ''; // restore if hidden from previous run
        typingQueue = [];
        isTyping = false;

        function showTabsArea() {
            if (!tabsShown) {
                // Stop and hide typing preview before going compact
                typingQueue = [];
                isTyping = false;
                typingArea.textContent = '';
                typingArea.closest('div').style.display = 'none';
        ...
        // Rebuild tab bar in correct BMC order based on what has arrived so far
        function rebuildTabs() {
            const bar = document.getElementById('tab-bar');
            const arrivedKeys = new Set(analysisData.map(d => d.type === 'kesimpulan' ? '__kesimpulan__' : d.blok));

            // Sort arrived items by BMC_ORDER
            const ordered = BMC_ORDER.filter(k => arrivedKeys.has(k));

            // Only re-render if order has changed
            const currentKeys = [...bar.querySelectorAll('[data-key]')].map(b => b.dataset.key);
            if (JSON.stringify(currentKeys) === JSON.stringify(ordered)) return;

            bar.innerHTML = '';
            ordered.forEach(key => {
                const obj = analysisData.find(d => (d.type === 'kesimpulan' ? '__kesimpulan__' : d.blok) ===
                    key);
                if (!obj) return;
                const isKesimpulan = key === '__kesimpulan__';
                const label = isKesimpulan ? '★ KESIMPULAN' : key.toUpperCase();

                const btn = document.createElement('button');
                btn.className = 'tab-btn' + (isKesimpulan ? ' kesimpulan-tab' : '');
                btn.textContent = label;
                btn.dataset.key = key;
                if (activeTab === key) btn.classList.add('active');
                btn.addEventListener('click', () => activateTab(key));
                bar.appendChild(btn);
            });

            // Auto-activate first tab only once
            if (!firstActivated && ordered.length > 0) {
                firstActivated = true;
                activateTab(ordered[0]);
            }
        }
        ...
        try {
            const response = await fetch(API_ENDPOINT, {
                ...
                body: JSON.stringify({
                    text: rawText,
                    mode: selectedMode,
                    file_name: (fileInput._selectedFile || fileInput.files[0])?.name ?? null,
                    user_id: null // ganti dengan nilai user_id yang sesuai, misal dari session/localStorage
                })
            });
                    ...
                    if (line.startsWith('RAW:')) {
                        ...
                        try {
                            ...
                            if (obj.type !== 'done') {
                                ...
                                // Normalize blok name to canonical casing
                                if (obj.type === 'blok') obj.blok = normalizeBlocKey(obj.blok);
                        ...
                        document.getElementById('footer-ts').textContent += `  ·  SAVED`;
                        loadHistory(); // refresh sidebar otomatis
    ...
    // --- Tab bar scroll arrows + custom scrollbar ---
    function initTabScroll() {
        const bar = document.getElementById('tab-bar');
        const track = document.getElementById('tab-scrollbar-track');
        const thumb = document.getElementById('tab-scrollbar-thumb');
        const leftWrap = document.getElementById('tab-scroll-left-wrap');
        const rightWrap = document.getElementById('tab-scroll-right-wrap');

        function update() {
            const scrollable = bar.scrollWidth > bar.clientWidth + 4;
            const atStart = bar.scrollLeft <= 4;
            const atEnd = bar.scrollLeft >= bar.scrollWidth - bar.clientWidth - 4;

            // Left arrow: show only when not at start
            leftWrap.style.display = scrollable && !atStart ? 'flex' : 'none';
            // Right arrow: show only when not at end
            rightWrap.style.display = scrollable && !atEnd ? 'flex' : 'none';
    ...
    function switchView(view) {
        currentView = view;
        document.getElementById('view-analyzer').style.display = view === 'analyzer' ? 'block' : 'none';
        document.getElementById('view-history').style.display = view === 'history' ? 'flex' : 'none';

        // Update sidebar active state
        document.querySelectorAll('.history-item').forEach(el => {
            el.classList.toggle('active', view === 'history' && el.dataset.id === String(currentHistoryId));
        });
    }
    ...
    async function loadHistory() {
        try {
            const HISTORY_API = '<?= base_url() ?>bmc/history'; // GET → [{id, file_name, mode, created_at}, ...]
            ...
            // Keep empty placeholder, remove old items
            ...
            json.data.forEach(item => {
                ...
                const name = item.file_name ?
                    item.file_name.replace(/\.pdf$/i, '') :
                    `Analisis #${item.id}`;

                el.innerHTML = `
                    <div class="hi-name" title="${item.file_name || ''}">${name}</div>
                    <div class="hi-meta">
                        <span class="hi-mode ${item.mode}">${item.mode}</span>
                        <span>${date} ${time}</span>
                    </div>`;

                el.addEventListener('click', () => openHistory(item.id, name, item.mode, item.created_at));
                list.appendChild(el);
            });
    ...
    async function openHistory(id, name, mode, createdAt) {
        currentHistoryId = id;
        switchView('history');
        ...
        document.getElementById('history-detail-meta').textContent =
            `${mode.toUpperCase()} · ${new Date(createdAt).toLocaleString('id-ID')}`;

        // Show loading
        const loadingEl = document.getElementById('history-loading');
        loadingEl.style.display = 'flex';
        document.getElementById('history-tab-bar').innerHTML = '';
        document.getElementById('history-tab-content').innerHTML = '';

        try {
            const res = await fetch(`<?= base_url() ?>bmc/result/${id}`);
            const json = await res.json();
            ...
            loadingEl.style.display = 'none';
            renderHistoryTabs(json.data);

        } catch (err) {
            loadingEl.style.display = 'none';
            document.getElementById('history-tab-content').innerHTML =
                `<p class="mono" style="color:var(--rose);font-size:12px;">ERROR: ${err.message}</p>`;
        }
    }

    function renderHistoryTabs(data) {
        const bar = document.getElementById('history-tab-bar');
        ...
        function activateHistoryTab(key) {
            activeHistoryTab = key;
            bar.querySelectorAll('[data-key]').forEach(b => {
                b.classList.toggle('active', b.dataset.key === key);
            });
            // Render content — reuse existing renderTabContent logic
            // Temporarily swap analysisData, render, restore
            const prev = analysisData;
            analysisData = data;
            renderTabContent(key, content);
            analysisData = prev;
        }

        BMC_ORDER.forEach(key => {
            const obj = data.find(d =>
                (d.type === 'kesimpulan' ? '__kesimpulan__' : normalizeBlocKey(d.blok)) === key
            );
            if (!obj) return;
            ...
            btn.textContent = label;
            btn.dataset.key = key;
            btn.addEventListener('click', () => activateHistoryTab(key));
            bar.appendChild(btn);

            if (!firstKey) firstKey = key;
        });

        if (firstKey) activateHistoryTab(firstKey);
        initHistoryTabScroll();
    }

    // ── History tab scroll (mirror of initTabScroll) ─────────────────
    function initHistoryTabScroll() {
        const bar = document.getElementById('history-tab-bar');
        const track = document.getElementById('history-tab-scrollbar-track');
        const thumb = document.getElementById('history-tab-scrollbar-thumb');
        const lWrap = document.getElementById('history-tab-scroll-left-wrap');
        const rWrap = document.getElementById('history-tab-scroll-right-wrap');
        if (!bar) return;
        ...
        function update() {
            ...
            const atStart = bar.scrollLeft <= 4;
            const atEnd = bar.scrollLeft >= bar.scrollWidth - bar.clientWidth - 4;
            lWrap.style.display = scrollable && !atStart ? 'flex' : 'none';
            rWrap.style.display = scrollable && !atEnd ? 'flex' : 'none';
            track.style.display = scrollable ? 'block' : 'none';
            if (scrollable) {
                const ratio = bar.clientWidth / bar.scrollWidth;
                const maxScroll = bar.scrollWidth - bar.clientWidth;
    ...
    // ── Load hasil dari DB dan render ulang ──────────────────────────
    // Panggil: loadFromDB(42) untuk render hasil dengan ID 42
    async function loadFromDB(id) {
        const API = `<?= base_url() ?>bmc/result/${id}`;

        try {
            // Reset UI ke loading state dulu
            document.getElementById('empty-state').style.display = 'none';
            document.getElementById('tabs-area').style.display = 'none';
            const loadingEl = document.getElementById('loading-state');
            loadingEl.classList.remove('progress-compact');
            loadingEl.style.display = 'flex';
            loadingEl.style.opacity = '';
            document.getElementById('loading-text').textContent = 'Loading saved result...';
            document.getElementById('progress-bar').style.width = '40%';
            document.getElementById('progress-pct').textContent = '40%';
            ...
            // Feed ke analysisData — persis seperti saat streaming
            ...
            setTimeout(() => {
                ...
                document.getElementById('tab-content').innerHTML = '';
                document.getElementById('tabs-area').style.display = 'flex';
                rebuildTabsFromData(); // rebuild semua tab sekaligus
                document.getElementById('footer-ts').textContent =
                    `LOADED #${json.id} · ${json.mode.toUpperCase()} · ${json.created_at}`;
            }, 300);
    ...
    // Rebuild semua tab sekaligus dari analysisData yang sudah terisi penuh
