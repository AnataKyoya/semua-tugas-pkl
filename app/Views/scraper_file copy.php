            ...
            function htmlFileManager() {
                return {
                        ...
                        uploaded.forEach(file => {
                            if (file.name.toLowerCase().endsWith('.html')) {
                                this.files.push({
                                    ...
                                    progress: 0,
                                    errorMessage: '',
                                    csvLink: null // Menyimpan link atau path csv dari server
                                });
                            }
                        });
                        event.target.value = '';
                    ...
                    downloadCsv(file) {
                        ...
                        // Logic download: Jika csvLink berupa URL lengkap dari server
                        const link = document.createElement('a');
                        link.href = file.csvLink;
                        // link.setAttribute('download', file.customName.replace('.html', '') + '.csv');
                    ...
                    async submitConfigs() {
                        ...
                        for (let i = 0; i < this.files.length; i++) {
                            ...
                            try {
                                ...
                                // Pemetaan stage tanpa property 'next'
                                const stagesData = fileItem.stages.map(s => {
                                    ...
                                    return {
                                        'name': s.type, // Isinya 'list' atau 'detail' dari select UI
                                        'parent': s.parent,
                                        'fields': mappedFields
                                    };
                                });
