...
<body class="min-h-screen text-slate-900 flex flex-col">
            ...
            <div class="space-y-4">
                ...
                <div id="dropZone"
                    ...
                    <div class="flex items-center justify-center gap-3">
                        ...
                        <div class="text-left">
                            <p class="text-xs font-bold text-slate-700 leading-tight">Click or drag files (Max. 15MB dan
                                Max. 4 Halaman)</p>
            ...
            <div class="max-w-4xl mx-auto w-full">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <div id="mergeButtonContainer" class="hidden dropdown relative">
                            <button
                                class="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg transition-all shadow-sm">
                            ...
                            <div
                                class="dropdown-content absolute right-0 w-48 bg-white border border-slate-100 shadow-xl rounded-xl z-30 p-2">
                                <button onclick="mergeSelectedAs('pdf')"
                                    class="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg">
                                    Merge as PDF
                                </button>
                                <button onclick="mergeSelectedAs('docx')"
                                    class="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg">
                                    Merge as DOCX
                                </button>
                                <button onclick="mergeSelectedAs('xlsx')"
                                    class="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg">
                                    Merge as XLSX
                                </button>
                                <button onclick="mergeSelectedAs('json')"
                                    class="w-full text-left px-3 py-2 text-xs font-bold text-indigo-500 hover:bg-indigo-50 rounded-lg border-t border-slate-50 mt-1">
                                    Merge as JSON
                                </button>
                            </div>
                        </div>
    ...
    <script>
        ...
        // Global state untuk selected results
        ...
        function updateMergeButton() {
            ...
            if (selectedResults.size > 0) {
                ...
                countEl.textContent = selectedResults.size;
            } else {
        ...
        async function mergeSelectedAs(format) {
            if (selectedResults.size === 0) {
                showError('No Selection', 'Pilih minimal 1 file untuk di-merge.', 'warning');
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // window.location.href = `/file-crawler/downloadas/${data.merged_id}/${format}/${data.merged_id}`;
                    ...
                    throw new Error(data.error || 'Merge gagal');
                }
            } catch (error) {
                showError('Merge Error', error.message, 'error');
            } finally {
        ...
        async function sendToBackend(rawText) {
            const API_ENDPOINT = "<?= base_url() ?>file-crawler/process"; // Ganti dengan URL backend-mu
            const spliter = rawText.split("<prompt>")
        ...
        fileInput.onchange = async (e) => {
            ...
            for (const file of files) {
                // Cek duplikat
                // const isDupe = arrayFiles.some(f => f.name === file.name && f.size === file.size);
                // if (isDupe) continue;

                const ok = await checkPageCount(file);
                if (ok) arrayFiles.push(file);
            }
        ...
        async function checkPageCount(file) {
            ...
            if (totalPages > 4) {
                showError(
                    ...
                    `Maksimal 4 halaman per file.<br/>- ${file.name} (${totalPages} halaman)`,
                    'warning'
                );
                return false;
            }

            // Cek apakah total + halaman baru melebihi 4
        ...
        function updateFileList() {
            ...
            if (fileCountEl) fileCountEl.textContent = `${arrayFiles.length} Files`;
            ...
            if (arrayFiles.length > 4) {
                ...
                console.log(arrayFiles, "o")
                updateFileList();

                return;
            }

            if (arrayFiles.length > 0 && arrayFiles.length <= 4) {
                if (listEl) {
                    listEl.innerHTML = arrayFiles.map((f, i) => `
        ...
        document.getElementById('processBtn').onclick = async () => {
            ...
            try {
                ...
                console.log(fullTextParse);
                const aiResult = await sendToBackend(fullTextParse + "\n<prompt>\n" + document.getElementById(
                    'promptInput').value);
        ...
        function updateUI() {
            ...
            container.innerHTML = allResults.map(res => `
                <div id="card-${res.id_file}" class="result-card bg-white border border-slate-200 p-5 rounded-2xl shadow-sm hover:shadow-md transition-all">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <div class="dropdown relative">
                            ...
                            <div class="dropdown-content absolute right-0 w-48 bg-white border border-slate-100 shadow-xl rounded-xl z-30 p-2">
                                <button onclick="downloadAs('${res.id_file}', 'pdf', '${res.name}')" class="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg">Export PDF</button>
                                <button onclick="downloadAs('${res.id_file}', 'docx', '${res.name}')" class="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg">Export DOCX</button>
                                <button onclick="downloadAs('${res.id_file}', 'xlsx', '${res.name}')" class="w-full text-left px-3 py-2 text-xs font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 rounded-lg">Export XLSX</button>
                                <button onclick="downloadAs('${res.id_file}', 'json', '${res.name}')" class="w-full text-left px-3 py-2 text-xs font-bold text-indigo-500 hover:bg-indigo-50 rounded-lg border-t border-slate-50 mt-1">Raw JSON Data</button>
                            </div>
                        </div>
                    </div>
